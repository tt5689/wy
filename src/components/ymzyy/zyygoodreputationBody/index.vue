<template>
  <div class="zyygoodreputationMain">
    <!-- 背景图 -->
    <div class="hpBanner">
      <img src="https://yanxuan.nosdn.127.net/8b9e910d85d1c7799b782f2bc845d2c9.png  " />
      <div class="hpBannerTense">
        <span>欢迎来到网易严选</span>
      </div>
    </div>
    <!-- 导航栏 -->
    <div class="hpnav">
      <div class="hpnavNetx">
        <div class="hpnavAll" v-if="!flage">
          <span class="hpnavAll1">全部类目</span>
          <ul class="hpnavAll2">
            <li
              class="hprxnavNetxList1"
              v-for="(item,index) in navList"
              :key="index"
              @click="navListtab(index)"
              :class="rxnavIndex == index?'navactive3':''"
            >{{item}}</li>
          </ul>
        </div>
        <ul class="hpnavul" v-if="flage">
          <li
            class="hprxnavNetxList"
            v-for="(item,index) in navList"
            :key="index"
            @click="navListtab(index)"
            :class="rxnavIndex == index?'navactive3':''"
        ...
        <div class="hpnavDown" @click="handleNavDown()">
          <i class="iconfont iconxiala" :class="flage?'hpnavDownI1':'hpnavDownI2'"></i>
        </div>
      </div>
    </div>
    <!-- 主体 -->
    <div class="hpmain">
        <ul>
            <li class="hpmainList" v-for="(item,index) in zyygoodreputationList" :key="index">
                <div class="hplistTop">
                    <img
                    class="hplistIMG"
                    :src="item.listPicUrl"
                    ...
                    <div class="hptopRightOne" v-show="item.couponAvaliable"></div>
                    <div class="hptopRightTwo">{{item.name}}</div>
                    <div class="hptopRightThree">{{item.discountTag}}</div>
                    <div class="hptopRightPrice">
                ...
                <div class="hplistBottom">
                    <span>{{item.simpleDesc}}</span>
                </div>
            </li>
        </ul>
      <div class="hpmainMore">
        <h2 class="hpmoreTitle">更多榜单</h2>
        <ul class="hpmoreList">
          <li v-for="(item,index) in moretwo" :style="item.bgcs" :key="index">
            <div class="hpmoreListLeft">
              <span>{{item.titles}}</span>
              <i>进入榜单&gt;</i>
            </div>
            <img :src="item.imgss" class="hpmoreListRight"/>
          </li>
        </ul>
      </div>
    </div>
...
<script>
import {gteZyygoodreputation} from "api/zyygoodreputation"
export default {
    name:"ZyynewproductBody",
    // await要在anync里用，anync代表异步,代表当前函数是异步函数
    async created() {
        let data = await gteZyygoodreputation();
        this.zyygoodreputationList = data.data;
         
    },
    data() {
        return {
            zyygoodreputationList:[],
            navList:["全部","居家生活","运动旅行","全球特色","个护清洁","母婴亲子","服饰鞋包"], 
            flage:'ture',
            rxnavIndex:0,
            moretwo:[
