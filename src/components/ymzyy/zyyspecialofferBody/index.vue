<template>
  <div class="zyyspecialofferMain">
    <!-- 背景图 -->
    <div class="thBanner">
      <img src="https://yanxuan.nosdn.127.net/86657f876ca090f51a9aa75227e2bd73.png" />
      <div class="thBannerTense">
        <span>欢迎来到网易严选</span>
      </div>
    </div>
    <!-- 导航栏 -->
    <div class="thnav">
      <div class="thnavNetx">
        <div class="thnavAll" v-if="!flage">
          <span class="thnavAll1">全部类目</span>
          <ul class="thnavAll2">
            <li
              class="thrxnavNetxList1"
              v-for="(item,index) in navList"
              :key="index"
              @click="navListtab(index)"
              :class="rxnavIndex == index?'navactive1':''"
            >{{item}}</li>
          </ul>
        </div>
        <ul class="thnavul" v-if="flage">
          <li
            class="thrxnavNetxList"
            v-for="(item,index) in navList"
            :key="index"
            @click="navListtab(index)"
            :class="rxnavIndex == index?'navactive1':''"
        ...
        <div class="thnavDown" @click="handleNavDown()">
          <i class="iconfont iconxiala" :class="flage?'thnavDownI1':'thnavDownI2'"></i>
        </div>
      </div>
    </div>
    <!-- 主体 -->
    <div class="thmain">
        <ul>
            <li class="thmainList" v-for="(item,index) in zyyspecialofferList" :key="index">
                <div class="thlistTop">
                    <img
                    class="thlistIMG"
                    :src="item.listPicUrl"
                    />
                    <div class="thtopRight">
                    <div class="thtopRightOne">{{item.promTag}}</div>
                    <div class="thtopRightTwo">{{item.name}}</div>
                    <div class="thtopRightThree">{{item.discountTag}}</div>
                    <div class="thtopRightPrice">
                ...
                <div class="thlistBottom">
                    <span>{{item.simpleDesc}}</span>
                </div>
            </li>
        </ul>
      <div class="thmainMore">
        <h2 class="thmoreTitle">更多榜单</h2>
        <ul class="thmoreList">
          <li v-for="(item,index) in moretwo" :style="item.bgcs" :key="index">
            <div class="thmoreListLeft">
...
<script>
import {gteZyyspecialoffer} from "api/zyyspecialoffer"
export default {
    name:"ZyyspecialofferBody",
    // await要在anync里用，anync代表异步,代表当前函数是异步函数
    async created() {
        let data = await gteZyyspecialoffer();
        this.zyyspecialofferList = data.data
