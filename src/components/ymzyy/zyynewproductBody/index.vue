<template>
  <div class="zyynewproductrMain">
    <!-- 背景图 -->
    <div class="xpBanner">
      <img src="https://yanxuan.nosdn.127.net/75e820f2ae637e7d2a4f892958141763.png" />
      <div class="xpBannerTense">
        <span>欢迎来到网易严选</span>
      </div>
    </div>
    <!-- 导航栏 -->
    <div class="xpnav">
      <div class="xpnavNetx">
        <div class="xpnavAll" v-if="!flage">
          <span class="xpnavAll1">全部类目</span>
          <ul class="xpnavAll2">
            <li
              class="xprxnavNetxList1"
              v-for="(item,index) in navList"
              :key="index"
              @click="navListtab(index)"
              :class="rxnavIndex == index?'navactive2':''"
            >{{item}}</li>
          </ul>
        </div>
        <ul class="xpnavul" v-if="flage">
          <li
            class="xprxnavNetxList"
            v-for="(item,index) in navList"
            :key="index"
            @click="navListtab(index)"
            :class="rxnavIndex == index?'navactive2':''"
          >{{item}}</li>
        </ul>
        <div class="xpnavDown" @click="handleNavDown()">
          <i class="iconfont iconxiala" :class="flage?'xpnavDownI1':'xpnavDownI2'"></i>
        </div>
      </div>
    </div>
    <!-- 主体 -->
    <div class="xpmain">
        <ul>
            <li class="xpmainList" v-for="(item,index) in zyynewproductList" :key="index">
                <div class="xplistTop">
                    <img
                    class="xplistIMG"
                    :src="item.listPicUrl"
                    />
                    <div class="xptopRight">
                    <div class="xptopRightOne" v-show="item.couponAvaliable"></div>
                    <div class="xptopRightTwo">{{item.name}}</div>
                    <div class="xptopRightThree">{{item.discountTag}}</div>
                    <div class="xptopRightPrice">
                        <span>￥{{item.retailPrice}}</span>
                        <i>￥{{item.counterPrice}}</i>
                        <em>马上抢</em>
                    </div>
                    </div>
                </div>
                <div class="xplistBottom">
                    <span>{{item.simpleDesc}}</span>
                </div>
            </li>
        </ul>
      <div class="xpmainMore">
        <h2 class="xpmoreTitle">更多榜单</h2>
        <ul class="xpmoreList">
          <li v-for="(item,index) in moretwo" :style="item.bgcs" :key="index">
            <div class="xpmoreListLeft">
              <span>{{item.titles}}</span>
              <i>进入榜单&gt;</i>
            </div>
            <img :src="item.imgss"/>
          </li>
        </ul>
      </div>
    </div>

    <!-- end -->
  </div>
</template>

<script>
import {gteZyynewproductBody} from "api/zyynewproduct"
export default {
    name:"ZyynewproductBody",
    // await要在anync里用，anync代表异步,代表当前函数是异步函数
    async created() {
        let data = await gteZyynewproductBody();
        this.zyynewproductList = data.data
         
    },
    data() {
        return {
            zyynewproductList:[],
            navList:["全部","居家生活","服饰鞋包","美食酒水","个护清洁","母婴亲子","运动旅行"], 
            flage:'ture',
            rxnavIndex:0,
            moretwo:[
                {   imgss:"https://yanxuan.nosdn.127.net/b4949f469d564ab34e69ebc45a6233ec.png",
                    bgcs:{backgroundColor:"#f1f8fd"},
                    titles:"居家生活榜"
                },
                {   imgss:"https://yanxuan.nosdn.127.net/ce4e606b4ddd2edebf57f63b56256813.png",
                    bgcs:{backgroundColor:"#f3f8ee"},
                    titles:"服饰鞋包榜"
                },
                {   imgss:"https://yanxuan.nosdn.127.net/9dd811d3b258d9a1ef6a84254a0da18e.png",
                    bgcs:{backgroundColor:"#fdf3ea"},
                    titles:"美食酒水榜"
                },
                {   imgss:"https://yanxuan.nosdn.127.net/8f3798ffdabcacb525bc5dfa88bd4b4e.png",
                    bgcs:{backgroundColor:"#fff6e5"},
                    titles:"个人清洁榜"
                },
                {   imgss:"https://yanxuan.nosdn.127.net/2f0620f2f41555d24b2645d4bcff451e.png",
                    bgcs:{backgroundColor:"#f4f9fd"},
                    titles:"母婴亲子榜"
                },
                {   imgss:"https://yanxuan.nosdn.127.net/17c904bff54e6e550c6d24632e1acd8d.png",
                    bgcs:{backgroundColor:"#f3f8ee"},
                    titles:"运动旅行榜"
                },
                {   imgss:"https://yanxuan.nosdn.127.net/aa1ec3b84c88e959df8dcc4588d002ed.png",
                    bgcs:{backgroundColor:"#f1f8fd"},
                    titles:"数码家电榜"
                },
                {   imgss:"https://yanxuan.nosdn.127.net/6f3c97245dde19b9017a04045128ca7e.png",
                    bgcs:{backgroundColor:"#f3f8ee"},
                    titles:"全球特色榜"
                },
            ]
        }
    },
    methods: {
        navListtab(index) {
            this.rxnavIndex = index;                
        },
        handleNavDown() {                
            this.flage = !this.flage;   
        }
    },
}
</script>

<style scope>
   .zyynewproductrMain {
    background-color: #eee;
    flex: 1;
    overflow-x: hidden;
    overflow-y: auto;
    display: flex;
    flex-wrap: wrap;
    min-height: 5rem;
    }
    .xpBanner {
        width: 100%;
        position: absolute;
        margin-top: .88rem;
    }
    .xpBanner img {
        width: 100%;
    }
    .xpBannerTense {
        border-radius: .2rem;
        width: 2.73rem;
        height: .4rem;
        background-color: #345842;
        opacity: 0.7;
        display: flex;
        justify-content: center;
        position: absolute;
        top:45%;
        left:50%;
        transform: translate(-50%,-50%);
    }
    .xpBannerTense span {
        color: #fff;
        width: 2rem;
        height: 100%;
        line-height: .4rem;
        opacity: 1;
        font-size: .12rem;
        text-align: center;
    }
    /* 导航 */
    .xpnav {
        width: 100%;
        height: 1rem;
        display: flex;
        position: absolute;
        top: 3.9rem;
        justify-content: center;    
    }
    .xpnavAll {
        float: left;
        width: 6.1rem;
        height: 1rem;
    }
    .xpnavNetx {
        width: 7.1rem;
        height: 100%;
        background-color: #fff;
        border-radius: .2rem .2rem 0 0;
    }
    .xpnavAll1 {
        width: 100%;
        height: 1rem;
        line-height: 1rem;
        color: #333;
        font-size: .3rem;
        margin-left: .3rem
    }
    .xpnavul {
        float: left;
        width: 6rem;
        margin-left: .1rem;
        height: 100%;
        display: flex;
        align-items: center;
        overflow-x: auto;
        overflow-y: hidden;
    }
    .xpnavAll2 {
        width: 7.1rem;
        height: 2.48rem;
        background-color: #fff;
        position:absolute;
        z-index: 10;
        top: .9rem;
        border-top: 1px solid #dfdfe0;
        border-radius:  0 0 .2rem .2rem;
        box-shadow: 0 7px 5px -5px;
    }
    .xprxnavNetxList1 {
        display: inline-block;
        color: #454545;
        font-size: .26rem;
        min-width: 1.4rem;
        height: .4rem;
        line-height: .4rem;
        text-align: center;
        margin-top: .4rem;
        margin-left: .2rem;
        border: 1px solid #bbb;
        border-radius: .2rem;
    }
    .xprxnavNetxList {
        display: inline-block;
        color: #454545;
        font-size: .26rem;
        min-width: 1.4rem;
        height: .4rem;
        line-height: .4rem;
        text-align: center;
        flex: 0 0 auto;
        box-sizing: border-box;
        margin-left: .2rem;
    }
    .xpnavDown {
        float: left;
        width: 1rem;
        height: 100%;
        background-color: #fff;
        opacity: 0.5;
        border-radius: 0 .2rem 0 0;
        text-align: center;
        line-height: 1rem;
    }
    .xpnavDownI1 {
        display: block;
        width: 100%;
        height: 100%;
        transition: all 1s;
    }
    .xpnavDownI2 {
        display: block;
        width: 100%;
        height: 100%;
        transform:rotate(180deg);
        transition: all 1s;
    }
    /* 隐藏滚动条 */
    ::-webkit-scrollbar {display:none} 
    .navactive2 {
        background-color: #e02e35;
        border-radius: .2rem;
        color: #fff;
    }
    /* 主体 */
    .xpmain {
        width: 7.1rem;
            margin: 4.8rem auto 0;
        min-height: 1rem;
        display: flex;
        flex-direction:column;
        align-items: center;
        background-color: #eee;
        z-index: 5;
    }
    .xpmainList {
        box-sizing: border-box;
        height: 3.84rem;
        width: 7.1rem;
        background-color: #fff;
        border-top: 1px solid #dfdfe0;
        display: flex;
        flex-direction: column;
        align-items: center;
        justify-content: space-around;
    }
    .xplistTop {
        display: flex;
        height: 2.4rem;
    }
    .xplistIMG {
        width: 2.4rem;
        height: 2.4rem;
    }
    .xptopRight {
        width: 4.06rem;
        height: 2.4rem;
        margin-left: .2rem;
        display: flex;
        flex-direction: column;
        justify-content: space-between;
        
    }
    .xptopRightOne {
        height: .3rem;
        width: 1.5rem;
        line-height: .3rem;
        font-size: .14rem;
        color: #b4282d;
        border: 1px solid #b4282d;
        text-align: center;
    }
    .xptopRightTwo {
        height: .96rem;
        line-height: .48rem;
        width: 3.6rem;
        font-size: .3rem;
        color: #333;
    }
    .xptopRightThree {
        height: .39rem;
        line-height: .39rem;
        width: 3.6rem;
        color: forestgreen;
    }
    .xptopRightPrice {
        width: 4.06rem;
        height: .5rem;
        line-height: .5rem;
    }
    .xptopRightPrice span {
        color: #DD1A21;
        font-size: .32rem;
        float: left;
        width: 1.2rem;
        height: .3rem;
        text-align: center;
        line-height: .3rem;
    }
    .xptopRightPrice i {
        line-height: .4rem;
        width: 1rem;
        height: .3rem;
        float: left;
        color: #999999;
        font-size: .24rem;
        text-decoration:line-through
    }
    .xptopRightPrice em {
        float: right;
        text-align: center;
        line-height: .43rem; 
        background-color: #e03636;
        height: .43rem;
        width: 1.04rem;
        color: #fff;
        border-radius: .2rem;
    }
    .xplistBottom {
        width: 6.58rem;
        height: .6rem;
        background-color: #f6f6f6;
        border-radius: .5rem;
        display: flex;
        justify-content: center;
        overflow-x: auto;
        overflow-y: hidden;
    }
    .xplistBottom span {
        line-height: .6rem;
        color: #333;
        font-size: .28rem;
        padding: 0 .3rem;
    }
    .xpmainMore {
        display: flex;
        flex-direction: column;
        align-items: center;
        width: 100%;
        min-height: .5rem;
        margin-top: .2rem;
    }
    .xpmoreTitle {
        width: 7.1rem;
        height: .9rem;
        background-color: #fff;
        box-sizing: border-box;
        text-align: center;
        line-height: .9rem;
    }
    .xpmoreList {
        width: 6.95rem;
        height: 100%;
        background-color: #fff;
        padding-bottom: 1.05rem;
        padding-left: .15rem;
    }
    .xpmoreList li {
        width: 3.2rem;
        height: 1.3rem;
        float: left;
        display: flex;
        justify-content: space-around;
        margin: .1rem;
    }
    .xpmoreListLeft {
        display: flex;
        flex-direction: column;
        justify-content: center;
    }
    .xpmoreListLeft i {
        font-size: .16rem;
    }
    .xpmoreListLeft li img {
        width: 1.3rem;
        height: 1.3rem;
    }
</style>